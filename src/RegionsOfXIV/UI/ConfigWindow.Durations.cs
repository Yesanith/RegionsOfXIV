using Dalamud.Bindings.ImGui;
using Dalamud.Interface.Utility.Raii;
using RegionsOfXIV.Services;

namespace RegionsOfXIV.UI;

internal sealed partial class ConfigWindow
{
    // "settled" rather than "changed" drives the preview here: replaying the notification on
    // every frame of a drag would restart the animation continuously and show nothing useful, so
    // it fires once when a slider is let go.
    private void DrawDurationsTab()
    {
        using var tab = ImRaii.TabItem(Loc.Label("durations.tab", "Durations"));
        if (!tab) return;

        var changed = false;

        var settled = false;

        UiText.Wrapped(Loc.Get(
            "durations.intro",
            "The line arrives, lands, then decodes. Motion and decode are separate " +
            "stages and each takes its own time."));
        ImGui.Separator();

        this.config.FadeInDuration = DrawSeconds(
            Loc.Label("durations.fadein", "Fade in"),
            this.config.FadeInDuration, 0.05f, 3f, ref changed, ref settled);
        UiText.Tooltip(Loc.Get(
            "durations.fadein.tooltip",
            "How long the line takes to come up to full strength as it appears."));

        using (ImRaii.Disabled(this.config.Motion == MotionEffect.None))
        {
            this.config.MotionDuration = DrawSeconds(
                Loc.Label("durations.motion", "Motion"),
                this.config.MotionDuration, 0.1f, 5f, ref changed, ref settled);
        }

        UiText.Tooltip(Loc.Get(
            "durations.motion.tooltip",
            "How long the letters take to arrive. Runs alongside the fade in,\n" +
            "and does nothing when the motion on the Effects tab is None."));

        using (ImRaii.Disabled(!this.config.DecodeEffectEnabled))
        {
            this.config.RevealDuration = DrawSeconds(
                Loc.Label("durations.decode", "Decode"),
                this.config.RevealDuration, 0.05f, 5f, ref changed, ref settled);
        }

        UiText.Tooltip(Loc.Get(
            "durations.decode.tooltip",
            "How long the Eorzean takes to resolve, once the letters have landed.\n" +
            "Needs the decode effect on the Effects tab."));

        this.config.ShowDuration = DrawSeconds(
            Loc.Label("durations.hold", "Hold"),
            this.config.ShowDuration, 0.5f, 15f, ref changed, ref settled);
        UiText.Tooltip(Loc.Get(
            "durations.hold.tooltip",
            "How long the finished line stays up before it starts to fade."));

        this.config.FadeOutDuration = DrawSeconds(
            Loc.Label("durations.fadeout", "Fade out"),
            this.config.FadeOutDuration, 0.05f, 5f, ref changed, ref settled);
        UiText.Tooltip(Loc.Get(
            "durations.fadeout.tooltip",
            "How long the line takes to disappear once its time is up."));

        if (changed)
            MarkUnsaved();

        if (settled)
            this.actions.Preview(Sample);
    }
}
